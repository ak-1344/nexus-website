...
export function MobileRadialNav({ navItems }: MobileRadialNavProps) {
  ...
  return (
    <div className="md:hidden">
      ...
      <div className="fixed bottom-6 right-6 z-[65] flex flex-col items-center">
        <motion.button
          whileTap={{ scale: 0.92 }}
          onClick={() => setOpen((v) => !v)}
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed bottom-6 right-6 z-[65] pointer-events-auto"
            >
              <div className="relative w-20 h-20 flex items-center justify-center">
                {/* Nav Items positioned in a quarter circle around the main button */}
                {navItems.map((item, i) => {
                  // Create a quarter circle (90 degrees) starting from bottom-right
                  ...
                  const angle = Math.PI + (i * quarterAngleStep) // Start from left side (π) and go up
                  const x = Math.cos(angle) * 150 // Reduced radius to bring buttons closer to main N button
